...
export default function DriverMonitorPage() {
  ...
  const startMonitoring = async () => {
    ...
    try {
      setIsUploading(true)
      setProcessingMsg(t('adas.uploadingVideoToServerWait', { size: fileSizeMB }))
      ...
      setJobId(newJobId)
      const pct = typeof data.progress_percent === 'number' ? data.progress_percent : (typeof (data as any).progress === 'number' ? (data as any).progress : 0)
      setProgress(pct)
      setFatigueLevel(0)
      ...
      toast({ title: t('driverMonitor.monitoringStarted'), description: t('driverMonitor.analyzingVideo') })
      // Video không hiển thị trong lúc phân tích (overlay như ADAS), không cần play ở đây
    } catch (err: any) {
      console.error('startMonitoring error:', err)
      setProcessingMsg('')
      toast({ title: t('driverMonitor.apiError'), description: err?.message ?? String(err), variant: 'destructive' })
      setIsMonitoring(false)
    } finally {
    ...
    const pollResult = async () => {
      try {
        ...
        if (!res.ok) {
          ...
          console.warn('[DriverResult] GET /api/video/result/...', res.status, errBody)
          if (res.status === 404) {
        ...
        const pct = typeof data.progress_percent === 'number' ? data.progress_percent : (typeof data.progress === 'number' ? data.progress : undefined)
        if (typeof pct === 'number') setProgress(pct)

        const elapsed = Math.floor((Date.now() - startedAtRef.current) / 1000)
        const timeStr = elapsed >= 60 ? `${Math.floor(elapsed / 60)}:${(elapsed % 60).toString().padStart(2, '0')}` : `${elapsed}s`
        setProcessingMsg(t('adas.analyzingProgress', { progress: typeof pct === 'number' ? pct : 0, time: timeStr }))
        ...
        if (st === 'completed' || st === 'failed' || st === 'error') {
          ...
          if (st === 'completed') {
            if (data.video_filename) {
              const resultFilename = String(data.video_filename).replace(/\.mp4$/i, '_result.mp4')
              setVideoUrl(getApiUrl(API_ENDPOINTS.VIDEO_DOWNLOAD(jobId, resultFilename)))
          ...
          toast({
            title: st === 'completed' ? t('driverMonitor.jobCompleted') : t('driverMonitor.jobFailed'),
            description: data.error_message || (Array.isArray(data.recommendations) ? data.recommendations.join('. ') : data.recommendations) || undefined,
          })
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const pollDriverStatus = async () => {
      try {
        ...
        if (!st) return
        if (typeof st.fatigue_level === 'number') setFatigueLevel(Math.round(st.fatigue_level))
        if (typeof st.distraction_level === 'number') setDistractionLevel(Math.round(st.distraction_level))
        if (typeof st.eyes_closed === 'boolean') setEyesClosed(st.eyes_closed)
        if (typeof st.blink_rate === 'number') setBlinkRate(Math.round(st.blink_rate))
      } catch (err) {
            ...
            <GlassCard className="p-6">
              ...
              <div className="text-sm text-fg-secondary space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {t('driverMonitor.fatigueDetection')}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    {t('driverMonitor.distractionDetection')}
                  </span>
                </div>
                <div className="flex items-center gap-2">
          ...
          <GlassCard glow="green" className="xl:col-span-2 h-full p-6">
                ...
                <div className="absolute inset-0 glass-card flex flex-col items-center justify-center text-neon-cyan gap-4">
                  ...
                  <div className="w-full max-w-md px-8 space-y-2">
                    ...
                    <div className="flex justify-between text-xs text-fg-secondary">
                      <span>{progress}%</span>
                      <span>{isUploading ? 'Đang upload...' : 'Đang xử lý...'}</span>
                    </div>
                  </div>
                  {processingMsg && (
                  ...
                  {!isUploading && isMonitoring && (
                    <p className="text-xs text-fg-secondary">
                      {t('driverMonitor.analyzingStatus', {
                        ...
                        eyes: eyesClosed ? t('driverMonitor.eyesClosed') : t('driverMonitor.eyesOpen'),
                        blink: blinkRate,
                      })}
                    </p>
                  )}
            ...
            {isUploading || isMonitoring ? (
              <div className="mt-4 text-sm text-fg-secondary flex items-center gap-2">
                <AlertTriangle
                  className="h-10 w-10 text-red-500 animate-pulse [animation-duration:0.8s] drop-shadow-[0_0_8px_rgba(255,0,0,0.8)]"
                />
            ...
            {loadingVideos ? (
              ...
              <div className="flex flex-col items-center justify-center h-40 text-fg-secondary">
                ...
                <p className="text-sm mt-1">
                  Hãy upload video mới để bắt đầu.
                </p>
              </div>
            ) : (
