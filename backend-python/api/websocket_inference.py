...
class ObjectTracker:
    def __init__(self, iou_threshold=0.15, max_age=30):
        self.tracks = {}  # track_id -> {bbox, class, conf, age, smooth_bbox, frames_seen}
        self.next_id = 0
        self.iou_threshold = iou_threshold  # 0.15 = rất dễ match
        self.max_age = max_age  # 30 frames = 1.5 giây
        self.smoothing_factor = 0.95  # 0.95 = SIÊU SIÊU smooth
        self.min_frames_to_show = 8  # Phải 8 frames mới show - loại bỏ HOÀN TOÀN nháy
        self.confidence_threshold = 0.35  # Chỉ hiển thị detections > 35%
    ...
    def smooth_bbox(self, old_bbox, new_bbox):
        """Smooth bounding box transition with ULTRA strong dampening"""
        alpha = self.smoothing_factor  # 0.92 = extremely smooth
        return [
    ...
    def smooth_confidence(self, old_conf, new_conf):
        """Smooth confidence to prevent label flickering"""
        return old_conf * 0.9 + new_conf * 0.1  # 90% old, 10% new
    
    def update(self, detections):
        """Update tracks with new detections - with strong smoothing"""
        ...
        for det in detections:
            ...
            if best_track_id is not None:
                ...
                # Chỉ hiển thị nếu đã xuất hiện đủ frames
                if self.tracks[best_track_id]['frames_seen'] >= self.min_frames_to_show:
                    det['track_id'] = best_track_id
                    det['bbox'] = smooth_bbox  # Use smoothed bbox
                    det['conf'] = smooth_conf  # Use smoothed confidence
                    det['is_new'] = False
                    det['stable'] = True
                ...
                # Don't add to output yet - wait for confirmation
        
                # Age out unmatched tracks gradually
        tracks_to_remove = []
        for track_id in self.tracks:
            if track_id not in matched_tracks:
                self.tracks[track_id]['age'] += 1
                # Keep showing track for longer even if not detected (reduce flickering)
                if self.tracks[track_id]['age'] <= 8 and self.tracks[track_id]['frames_seen'] >= self.min_frames_to_show:
                    # Continue showing with last known position
                    det = {
                        'bbox': self.tracks[track_id]['smooth_bbox'],
                        'conf': self.tracks[track_id]['conf'] * 0.98,  # Slightly reduce confidence
...
def load_model(model_id: str):
    ...
    if model_id.startswith("yolov8"):
        ...
        # Default to yolov8n for generic "yolo" request
    ...
    if model_file and model_file.exists():
        if model_id.startswith("yolov8") or model_id == "yolo":
            try:
